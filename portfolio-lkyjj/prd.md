...
| 风险点 | 可能性 | 影响 | 应对方案 |
|--------|--------|------|----------|
| GitHub API调用限流 | 中 | 项目区无法加载开源项目 | 1. ✅ 已实现接口缓存（next revalidate 12小时）；<br>2. 配置GITHUB_TOKEN环境变量提升限流额度；<br>3. ✅ 已准备本地backup-repos.json降级方案 |
| 移动端兼容性问题 | 中 | 部分机型显示异常 | 1. ✅ 已使用Tailwind响应式布局；<br>2. 测试主流机型（iPhone/Android）；<br>3. ✅ 导航栏已适配移动端折叠菜单 |
| 部署后加载速度不达标 | 低 | 用户体验差，SEO受影响 | 1. ✅ Next.js SSG静态生成；<br>2. ✅ 图片使用WebP格式；<br>3. ✅ 已配置Vercel CDN加速 |
| 中文路径编码问题 | 低 | 本地开发调试困难 | 1. ✅ 已识别PowerShell中文路径问题；<br>2. 使用短路径或英文路径部署；<br>3. 生产环境使用Vercel无此问题 |
| 国际化切换状态同步 | 低 | 不同页面语言不一致 | 1. ✅ 已实现localStorage持久化；<br>2. ✅ 已实现languageChange事件跨组件同步；<br>3. ✅ 所有页面已支持中英文切换 |
